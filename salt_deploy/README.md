...
`current_link` must be a file directly inside `base_dir`, and `releases_dir`
must be inside it too. The promotion stages the new symlink beside the live one
and gives it a relative target, so that the swap is a single atomic rename;
neither property survives moving the link elsewhere. The formula refuses to
render the deployment script if either is configured otherwise.
...
`master:config_file` defaults to `/etc/salt/master.d/99-salt-deploy.conf`. Two
things depend on that name:

- Salt reads `master.d/*.conf` in sorted order and later files win, so the `99-`
  prefix puts this `file_roots` after anything the `salt` formula writes to
  `master.d/defaults.conf`.
- The `salt` formula deploys `master.d` with `clean: true`, which deletes files
  it does not manage. Its `salt:config_d_preserve` default covers `[0-9]*`, and
  `salt:config_d_preserve_from` additionally resolves
  `salt_deploy:master:config_file` by name.

Renaming this file to something without a numeric prefix gives up the ordering
and, on a `salt` formula older than that preserve mechanism, gets the file
silently deleted on the next highstate — after which the master serves an empty
`file_roots`.
...
than being interpolated into the command. A `cmd.run`'s command line is part of
its state return, and state returns end up in the master job cache — which on a
master configured for Alcali is a database more than one person can read.
`hide_output` does not cover this; it suppresses the command's output, not the
command.
...
A staged release is removed by an `EXIT` trap, which does not run if the
process is killed outright. Each deployment reaps `.staging.*` directories in
`releases/` older than a day; the age cut-off is what keeps it from removing a
staging directory belonging to a live run, though the exclusive lock means
